...
function renderRecipes(recipeList) {
	...
	if (recipeList.length === 0) {
		main.innerHTML = '<p class="no-results">No recipes found. Try a different search term.</p>';
		return;
	}

	main.innerHTML = recipeList.map(renderRecipeCard).join('');
}
...
function recipeMatchesTerm(recipe, term) {
	...
	return recipe.name.toLowerCase().includes(lowerTerm);
}
...
function filterRecipes(term) {
	...
	return recipes.filter(recipe => recipeMatchesTerm(recipe, trimmed));
}
...
document.addEventListener('DOMContentLoaded', () => {
	...
	renderRecipes([recipes[0]]);

	searchInput.addEventListener('input', () => {
		const results = filterRecipes(searchInput.value);
